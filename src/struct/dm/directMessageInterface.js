...
const Notification = require(`../notification/notificationHandler`)
...
class dmInterface extends Controller {
    ...
    async run() {
        ...
        if (!dmOptions[params[0]]) return this.reply(this.code.DM.UNAVAILABLE_OPTION, {
            field: this.meta.author
        })

        //  Returns if sub option is not available
        if (!dmOptions[params[0]].includes(params[1])) return this.reply(this.code.DM.UNAVAILABLE_OPTION, {
            field: this.meta.author
        })

        //  Returns if user has recently make changes
        if (await this.keyv.get(this.actionId)) return this.reply(this.code.DM.COOLING_DOWN, {
            field: this.meta.author
        })
