...
module.exports = {
    ...
    async Iexecute(client, reply, interaction, options, locale) {
		...
		collector.on(`collect`, async msg => {
			...
			// Handle if user asked to cancel the quest
			if ([`cancel`].includes(answer)) {
				reply.send(locale.QUEST.CANCEL,{followUp: true})
				msg.delete().catch(e => client.logger.warn(`fail to delete quest-answer due to lack of permission in GUILD_ID:${interaction.guild.id} > ${e.stack}`))
				quest.delete()
				client.db.redis.del(sessionID)
                return collector.stop()
			}
			//  Handle if the answer is incorrect
			if (answer !== activeQuest.correct_answer) return reply.send(locale.QUEST.INCORRECT_ANSWER, {deleteIn: 3, followUp: true})
            collector.stop()
			msg.delete().catch(e => client.logger.warn(`fail to delete quest-answer due to lack of permission in GUILD_ID:${interaction.guild.id} > ${e.stack}`))
			//  Update reward, user quest data and store activity to quest_log activity
