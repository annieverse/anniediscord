...
module.exports = {
	...
	async execute(client, reply, message, arg, locale) {
		...
		if (await client.db.redis.exists(sessionID)) return reply.send(locale.QUEST.SESSION_STILL_RUNNING, {
			socket: {
		...
		const fetching = await reply.send(locale.QUEST.FETCHING, {
			simplified: true,
			socket: {
		...
		collector.on(`collect`, async msg => {
			...
			if (answer !== activeQuest.correct_answer) return reply.send(locale.QUEST.INCORRECT_ANSWER, {
				deleteIn: 3
			})
			collector.stop()
			msg.delete().catch(e => client.logger.warn(`fail to delete quest-answer due to lack of permission in GUILD_ID:${message.guild.id} > ${e.stack}`))
			...
			client.db.updateInventory({
				itemId: 52,
				value: activeQuest.reward_amount,
