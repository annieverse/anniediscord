...
module.exports = {
    ...
    async execute(client, reply, message, arg, locale) {
        ...
        c.onAccept(async() => {
            ...
            client.db.userUtility.updateUserReputation(amount, targetUser.master.id, message.author.id, message.guild.id)
                //  Deduct gifts from sender
            client.db.databaseUtility.updateInventory({ itemId: gift.item_id, value: amount, operation: `-`, userId: message.author.id, guildId: message.guild.id })
    ...
    async Iexecute(client, reply, interaction, options, locale) {
        ...
        c.onAccept(async() => {
            ...
            if (gift.quantity < amount) return await reply.send(locale.GIFT.INSUFFICIENT_AMOUNT, {
                ...
                //  Adds reputation point to target user
            client.db.userUtility.updateUserReputation(amount, targetUser.id, interaction.member.id, interaction.guild.id)
                //  Deduct gifts from sender
            client.db.databaseUtility.updateInventory({ itemId: gift.item_id, value: amount, operation: `-`, userId: interaction.member.id, guildId: interaction.guild.id })
            return await reply.send(``, {
