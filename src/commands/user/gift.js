...
module.exports = {
	...
	async execute(client, reply, message, arg, locale) {
		...
		if (userLib.isSelf(targetUser.id)) return reply.send(locale.GIFT.SELF_TARGETING, {
			socket: {
		...
		if (!gift) return reply.send(locale.GIFT.MISSING_ITEM, {
			socket: {
				example: `e.g. **\`${client.prefix}gift ${targetUser.master.username} 10 ${availableGifts[0].name.toLowerCase()}\`**`
			}
		})
