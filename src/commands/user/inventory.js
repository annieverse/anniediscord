...
module.exports = {
    ...
    async execute(client, reply, message, arg, locale, prefix) {
        ...
        if (targetUserData.inventory.raw.length <= 0) return reply.send(INVALID_INVENTORY, {
            socket: {
                user: targetUser.username
            }
        })
        reply.send(locale.INVENTORY.FETCHING, {
                socket: {
                    emoji: await client.getEmoji(`AAUloading`)
                }
            })
            .then(async loading => {
