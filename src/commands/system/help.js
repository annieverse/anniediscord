...
module.exports = {
	...
	async execute(client, reply, message, arg, locale, prefix) {
		...
		const {
			isCategory,
			res
		} = await this.findCommandByKeyword(arg, client.commands.filter(node => !this.ignoreGroups.includes(node.group)))
	...
	async findCommandByKeyword(keyword = ``, src = {}) {
		...
		if (parents.includes(keyword)) return {
			isCategory: true,
			res: keyword
		}
		//  Find by command master name
		if (src.has(keyword)) return {
			isCategory: false,
			res: src.get(keyword)
		}
		//  Find by aliases
		const aliasSearch = src.filter(node => node.aliases.includes(keyword))
		if (aliasSearch.size > 0) return {
			isCategory: false,
			res: aliasSearch.first()
		}
		return {
			isCategory: false,
			res: null
