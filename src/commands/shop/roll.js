...
class Gacha {
	 ...
	 */
	async execute() {
		const { message, name, reply, code:{GACHA}, choice, emoji} = this.stacks

		//	Returns if user doesn't have any lucky ticket
		if (!this.data.lucky_ticket) return reply(GACHA.ZERO_TICKET)

		//	Returns if user trying to do multi-roll with owned less than 10 tickets
		if (this.data.lucky_ticket < this.roll_type) return reply(GACHA.INSUFFICIENT_TICKET)
