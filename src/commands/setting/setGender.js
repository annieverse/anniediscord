...
module.exports = {
    ...
    async execute(client, reply, message, arg, locale, prefix) {
        const userGender = await client.db.userUtility.getUserGender(message.author.id)
        ...
        !targetGender ? client.db.userUtility.updateUserGenderToneutral(message.author.id) :
            client.db.userUtility.updateUserGender(message.author.id, targetGender)
    ...
    async Iexecute(client, reply, interaction, options, locale) {
        const userGender = await client.db.userUtility.getUserGender(interaction.member.id)    
        ...
        const targetGender = malePool.includes(key.toLowerCase()) ? `m` :
            femalePool.includes(key.toLowerCase()) ? `f` :
                null
        !targetGender ? client.db.userUtility.updateUserGenderToneutral(interaction.member.id) :
            //  Update/register gender
            client.db.userUtility.updateUserGender(interaction.member.id, targetGender)
        return await reply.send(locale.SETGENDER.SUCCESSFUL, {
            status: `success`,
            socket: {
                emoji: await client.getEmoji(`789212493096026143`)
