...
module.exports = async (client, interaction, command) => {
    ...
    if (hasCommandPermission) {
        const commandCooldown = readCommandCooldown(client, command.name, interaction.user.id, interaction.guildId)
        if (commandCooldown.active) return sendCommandCooldown(client, interaction, interaction.user.id, interaction.user.username, commandCooldown.diff)
        clearPendingCommandCooldown = startCommandCooldown(client, commandCooldown.instanceId)
    }
    // Handle localization
    let userData
    try {
        userData = await client.db.userUtils.getUserLocale(interaction.user.id)
    } catch (error) {
    ...
    */
    ...
    let userValidated
    try {
        userValidated = await client.db.redis.sIsMember(`VALIDATED_USERID`, interaction.user.id)
    } catch (error) {
        if (clearPendingCommandCooldown) clearPendingCommandCooldown()
        throw error
    }
