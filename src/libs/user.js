...
class User {
	...
	async lookFor(target, localPool = null) {
		...
		try {
			let findByFullString = await collection.fetch({
		...
		for (let i = 0; i < this.normalizedArgs.length; i++) {
			...
			try {
				findByStringToken = await collection.fetch({
					query: token,
					limit: 1
				})
			...
			for (let x = 0; x < neighbors.length; x++) {
				...
				try {
					findByCombinedStringTokens = await collection.fetch({
						query: combinedTokens,
						limit: 1
					})
	...
	async requestMetadata(user) {
		...
		try {
			...
			if (rankData.length <= 0) rankData = [{
				NAME: `Unranked`,
				COLOR: Themes[usedTheme.alias].text,
