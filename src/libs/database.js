...
class Database {
	...
	useItem(itemId, userId = ``, guildId = ``) {
		...
		return this._query(`
			...
			WHERE 
				user_id = ?
				AND item_id = ?
				AND guild_id = ?`, `run`, [userId, itemId, guildId])
	}
	...
	registerUserRemindersMasterTable() {
		return this._query(`CREATE TABLE IF NOT EXISTS user_reminders (
			...
			)`, `run`, [], `Verifying table user_reminders`)
	}
	...
	registerAutoResponderMasterTable() {
		return this._query(`CREATE TABLE IF NOT EXISTS autoresponders (
			...
			)`, `run`, [], `Verifying table autoresponders`)
	}
	...
	initializeUserSelfCoverTable() {
		return this._query(`CREATE TABLE IF NOT EXISTS user_self_covers (
		    ...
		    REFERENCES users(user_id) 
			   ON DELETE CASCADE
			   ON UPDATE CASCADE)`, `run`, [], `Verifying quests user_self_covers`)
	}
	...
	setGuildBio(guildId = ``, bio = ``) {
		...
		return this._query(`
			...
			WHERE guild_id = ?`, `run`, [bio, guildId], `${fn} updating new bio for GUILD_ID ${guildId}`)
	}
	...
	async setCustomConfig({
		...
		this._query(`INSERT OR IGNORE INTO guilds (guild_id, name, bio) values (?, ?, 'I am an awsome server come join me')`, `run`, [guild.id, guild.name])

		this._query(`UPDATE guilds SET updated_at = CURRENT_TIMESTAMP WHERE guild_id = ?`, `run`, [guild.id])
		let config_id
		try {
			...
			this._query(`INSERT OR IGNORE INTO guild_configurations (config_id, config_code, guild_id) values (?, ?, ?)`, `run`, [config_id, config_code, guild.id])
		} catch (error) {
			this._query(`INSERT OR IGNORE INTO guild_configurations (config_code, guild_id) values (?, ?)`, `run`, [config_code, guild.id])
		}
		return this._query(`UPDATE guild_configurations SET updated_at = CURRENT_TIMESTAMP, customized_parameter = ?, set_by_user_id = ? 
			WHERE guild_id = ? AND config_code = ?`, `run`, [customized_parameter, set_by_user_id, guild.id, config_code])
	}
	...
	async mostUsedCommands(limit = 5) {
		...
		return this._query(`
			...
			LIMIT ?`, `all`, [limit], `Fetching most used commands in commands_log`)
	}
	...
	writeToThread(user_id, mod_id, guild_id, thread_id, text) {
		this._query(`
			...
			VALUES (datetime('now'), ?, ?, ?, ?, ?)`, `run`, [user_id, mod_id, guild_id, thread_id, text])
	}
	/**
	...
	getBlockedUsers() {
		return this._query(`
				...
				WHERE blocked = 1`, `all`, [], `getting blocked users`)
	}
	...
	getBlockedUserReason(user_id) {
		return this._query(`
				SELECT reason
				FROM modmail_blocked_users
				WHERE user_id = ?`, `get`, [user_id], `getting blocked user's reason`)
	}
	...
	getBlockedUsersList() {
		return this._query(`
			SELECT user_id
			FROM modmail_blocked_users`, `all`, [], `getting blocked users`)
	}
	...
	registerUserInBlockList(user_id) {
		this._query(`
			...
			VALUES (datetime('now'), ?, 0)`, `run`, [user_id])
	}
	...
	blockUser(user_id, reason = `The Moderator didn't supply a reason, if you would like to appeal this block please address it to the mods on the server or owner.`) {
		this._query(`
			UPDATE modmail_blocked_users 
			SET blocked = 1 AND reason = ?
			WHERE user_id = ?`, `run`, [user_id, reason], `blocking user`)
	}
	...
	isblockedUser(id) {
		this._query(`
				...
				FROM modmail_blocked_users
				WHERE user_id = ?
				AND blocked = 0`, `get`, [id], `checking if user is blocked`)
	}
	...
	async alreadyOpenThread(id, dm = true) {
		...
		if (dm) {
			search = await this._query(`
				...
				WHERE user_id = ?
				AND status = 'open'
				ORDER BY thread_id`, `get`, [id], `getting open thread`)
		} else if (!dm) {
			search = await this._query(`
				SELECT *
				FROM modmail_threads
	...
	updateChannel(id, threadId) {
		this._query(`
				...
				SET channel = ?
				WHERE thread_id = ?`, `run`, [id, threadId])
	}
	...
	getlogsForUser(userId) {
		return this._query(`
			...
			ORDER BY thread_id`, `all`, [userId], `getting logs for user`)
	}
	...
	makeNewThread(user_id, guild_id, thread_id, status, is_anonymous) {
		try {
			...
			VALUES (datetime('now'), ?, ?, ?, ?, ?)`, `run`, [user_id, guild_id, thread_id, status, is_anonymous])
		} catch (error) {
	...
	deleteLog(id) {
		this._query(`
			...
			WHERE thread_id = ?`, `run`, [id], `delete logs for specific thread`)
		this._query(`
	...
	getLogByThreadId(id) {
		return this._query(`
			...
			WHERE thread_id = ?`, `all`, [id], `getting logs for specific thread`)
	}
	...
	getThreadTicket(id) {
		return this._query(`
			SELECT *
			FROM modmail_threads
			WHERE thread_id = ?`, `get`, [id], `getting thread data for specific thread`)
	}
	...
	setUserExp(amount = 0, userId = ``, guildId = ``) {
		return this._query(`
			UPDATE user_exp 
			SET current_exp = ?
			WHERE user_id = ? AND guild_id = ?`, `run`, [amount, userId, guildId])
	}
	...
	async getStrikeRecords(userId = ``, guildId = ``) {
		return this._query(`
			...
			ORDER BY datetime(registered_at) DESC`, `all`, [userId, guildId], `Fetching strikes for USER_ID ${userId}`, true)
	}
	...
	async registerStrike({
		...
		return this._query(`
			...
			VALUES(datetime('now'), ?, ?, ?, ?)`, `run`, [user_id, reason, reported_by, guild_id], `Registering new strike entry for USER_ID ${user_id}`)
	}
	...
	async getUserPosts(userId = ``, guildId = ``) {
		return this._query(`
			...
			FROM user_posts
			WHERE user_id = ? AND guild_id = ?
			ORDER BY registered_at DESC`, `all`, [userId, guildId])
	}
	...
	registerPost({
		...
		return this._query(`
			...
			`run`, [userId, url, caption, channelId, guildId]
		)
	}
	...
	addHeart({
		...
		total_likes = total_likes + 1 
		WHERE url = ?`,
			`run`, [recently_liked_by, url])
	}

	checkVIPStatus(userId = ``, guildId = ``) {
		return this._query(`SELECT EXISTS (SELECT 1 FROM user_inventories WHERE item_id = 128 AND quantity = 1 AND user_id = $userId AND guildId = $guildId)`, `get`, {
			guildId: guildId,
			userId: userId
		})
	}
	...
	getPurchasableItems(sortBy = `items.type_id`) {
		return this._query(`
			...
			ORDER BY ? DESC`, `all`, [sortBy], `Looking up for purchasable items`)
	}
	...
	isValidItem(itemId, guildId) {
		...
		return this._query(`
			SELECT EXISTS(SELECT * FROM items WHERE itemId = $itemId AND owned_by_guild_id = $guildId)`, `get`, {
			itemId: itemId,
			guildId: guildId
	...
	nullifyExpBooster(userId = ``, guildId = ``) {
		return this._query(`
            ...
            WHERE user_id = ? AND guild_id = ?`, `run`, [userId, guildId], `EXP booster for USER_ID ${userId} in GUILd ${guildId} has been nullified.`)
	}
	...
	registerFeaturedPost({
		timestamp = 0,
		url = ``,
		author = ``,
		channel = ``,
		heart_counts = 0,
		last_heart_timestamp = Date.now()
	}) {
		this.client.run(`
	...
	initializeQuestTable() {
		return this._query(`CREATE TABLE IF NOT EXISTS quests (
			...
			'name' TEXT,
			'description' TEXT,
			'correct_answer' TEXT)`, `run`, [], `Verifying quests table`)
	}
	...
	initializeUserQuestTable() {
		return this._query(`CREATE TABLE IF NOT EXISTS user_quests (
		    ...
		    REFERENCES guilds(guild_id) 
			   ON DELETE CASCADE
			   ON UPDATE CASCADE)`, `run`, [], `Verifying user_quest table`)
	}
	...
	initializeQuestLogTable() {
		return this._query(`CREATE TABLE IF NOT EXISTS quest_log (
			...
			user_id TEXT,
			guild_id TEXT,
			answer TEXT)`, `run`, [], `Verifying quest_log table`)
	}
	...
	async initializeAffiliatesTable() {
		await this._query(`CREATE TABLE IF NOT EXISTS affiliates (
			...
			'invite_link' TEXT,
			'notes' TEXT)`, `run`, [], `Verifying table affiliates`)
		return true
	}
	...
	async getAffiliates() {
		return this._query(`
			SELECT *
			FROM affiliates`, `all`, [], `Fetching affiliates list`)
	}
	...
	async recoverOldData() {
		...
		await this._query(`
			UPDATE usercheck 
			SET lastdaily = datetime(lastdaily/1000, 'unixepoch')`, `run`, [], `Converting usercheck's lastdaily into a proper datetime`)
		await this._query(`
			UPDATE usercheck 
			SET repcooldown = datetime(repcooldown/1000, 'unixepoch')`, `run`, [], `Converting usercheck's repcooldown into a proper datetime`)
		...
		await this._query(`
			...
			WHERE users.user_id = usercheck.userId`, `run`, [], `Migrating usercheck's dailies into user_dailies`)
		await this._query(`
			...
			SELECT 
				userId, 
				reputations
			FROM userdata`, `run`, [], `Migrating userdata's reps into user_reputations`)
		await this._query(`
			...
			WHERE usercheck.userId = userdata.userId`, `run`, [], `Migrating userdata and usercheck exp data into user_exp`)
		await this._query(`
			...
			WHERE users.user_id = userartworks.userId`, `run`, [], `Migrating userartworks into user_posts`)
		...
		await this._query(`
			...
			WHERE userId NOT IN (
				SELECT user_id 
				FROM users
			)`, `run`, [], `Remove non-existent userId in userbadges (deprecated) table.`)
		...
		await this._query(`
			...
			WHERE user_id NOT IN (
				SELECT user_id 
				FROM users
			)`, `run`, [], `Remove non-existent user_id in item_inventory (deprecated) table.`)
		...
		await this._query(`
			...
			WHERE item_id NOT IN (
				SELECT item_id 
				FROM items
			)`, `run`, [], `Remove non-existent item in item_inventory (deprecated) table.`)
		...
		await this._query(`
			...
			)`, `run`, [], `Remove duplicate items in item_inventory.`)
		...
		await this._query(`
			...
			WHERE 
				users.user_id = item_inventory.user_id`, `run`, [], `Migrating item_inventory into user_inventories`)
		await this._query(`
			INSERT INTO user_inventories (
				user_id,
				item_id,
			...
			AND t1.cover != ''
			AND t1.cover != 'defaultcover1'`, `run`, [], `Migrating userdata's cover into user_inventories`)
		...
		await this._query(`
			...
			)`, `run`, [], `Set cover in_use value to 1 based on user-applied cover in userdata`)
		...
		await this._query(`
			...
			AND t1.interfacemode != ''`, `run`, [], `Migrating userdata's interfacemode into user_inventories`)
		await this._query(`
			INSERT INTO user_inventories (
				user_id,
				item_id,
			...
			AND t1.sticker != ''`, `run`, [], `Migrating userdata's sticker into user_inventories`)
		await this._query(`
			INSERT INTO user_inventories (
				user_id,
				item_id,
			...
			AND t1.expbooster != ''`, `run`, [], `Migrating usercheck's exp boosters into user_inventories`)
		...
		await this._query(`
			...
			AND t1.slot1 != ''`, `run`, [], `Migrating userbadges(slot1) into user_inventories`)
		await this._query(`
			INSERT INTO user_inventories (
				user_id,
				item_id,
			...
			AND t1.slot2 != ''`, `run`, [], `Migrating userbadges(slot2) into user_inventories`)
		await this._query(`
			INSERT INTO user_inventories (
				user_id,
				item_id,
			...
			AND t1.slot3 != ''`, `run`, [], `Migrating userbadges(slot3) into user_inventories`)
		await this._query(`
			INSERT INTO user_inventories (
				user_id,
				item_id,
			...
			AND t1.slot4 != ''`, `run`, [], `Migrating userbadges(slot4) into user_inventories`)
		await this._query(`
			INSERT INTO user_inventories (
				user_id,
				item_id,
			...
			AND t1.slot5 != ''`, `run`, [], `Migrating userbadges(slot5) into user_inventories`)
		await this._query(`
			INSERT INTO user_inventories (
				user_id,
				item_id,
			...
			AND t1.slot6 != ''`, `run`, [], `Migrating userbadges(slot6) into user_inventories`)
		await this._query(`
			INSERT INTO user_inventories (
				user_id,
				item_id,
			...
			AND t1.slotanime != ''`, `run`, [], `Migrating userbadges(slotanime) into user_inventories`)
		...
		await this._query(`
			...
			SELECT
				...
				relationType2
			FROM relationship
			WHERE relationType2 IS NOT NULL`, `run`, [], `Migrating relationship(A) into user_relationships`)
		await this._query(`
			INSERT INTO user_relationships (
				user_id_A,
				user_id_B,
			...
			SELECT
				...
				relationType1
			FROM relationship
			WHERE relationType1 IS NOT NULL`, `run`, [], `Migrating relationship(B) into user_relationships`)
		...
		await this._query(`
			...
			WHERE 
				...
				AND userdata.anime_link IS NOT NULL`, `run`, [], `Migrating userdata's anime link into user_socialmedias`)
	}
	...
	async verifyingTables() {
		...
		await this._query(`CREATE TABLE IF NOT EXISTS trading_trades (
			...
			'channel' TEXT NOT NULL UNIQUE DEFAULT 0)`, `run`, [], `Verifying table trading_trades`)
		...
		await this._query(`CREATE TABLE IF NOT EXISTS trading_transaction (
			...
			'user_two_item' TEXT NOT NULL)`, `run`, [], `Verifying table trading_transaction`)
		...
		await this._query(`CREATE TABLE IF NOT EXISTS trading_blocked_users (
			...
			'reason' TEXT DEFAULT 'The Moderator didnt supply a reason, if you would like to appeal this block please address it to the mods on the server or owner.')`, `run`, [], `Verifying table trading_blocked_users`)
		...
		await this._query(`CREATE TABLE IF NOT EXISTS user_dailies (
		   ...
		   )`, `run`, [], `Verifying table user_dailies`)
		...
		await this._query(`CREATE TABLE IF NOT EXISTS user_reputations (
		   ...
		   )`, `run`, [], `Verifying table user_reputations`)
		...
		await this._query(`CREATE TABLE IF NOT EXISTS user_inventories (
			...
			)`, `run`, [], `Verifying table user_inventories`)
		...
		await this._query(`CREATE TABLE IF NOT EXISTS user_posts (
		   ...
		   )`, `run`, [], `Verifying table user_posts`)
		...
		await this._query(`CREATE TABLE IF NOT EXISTS user_socialmedias (
		   ...
		   )`, `run`, [], `Verifying table user_socialmedias`)
		...
		await this._query(`CREATE TABLE IF NOT EXISTS user_relationships (
		   ...
		   REFERENCES relationships(relationship_id)
		   		ON DELETE CASCADE
		   		ON UPDATE CASCADE
		   )`, `run`, [], `Verifying table user_relationships`)
	   ...
	   */
		...
		await this._query(`CREATE TABLE IF NOT EXISTS guild_configurations (
			...
			)`, `run`, [], `Verifying table guild_configurations`)
		...
		await this._query(`CREATE TABLE IF NOT EXISTS relationships (
		   ...
		   )`, `run`, [], `Verifying table relationships`)
		...
		await this._query(`CREATE TABLE IF NOT EXISTS item_gacha (
		   ...
		   )`, `run`, [], `Verifying table item_gacha`)
		...
		await this._query(`CREATE TABLE IF NOT EXISTS item_types (
		   ...
		   )`, `run`, [], `Verifying table item_types`)
		...
		await this._query(`CREATE TABLE IF NOT EXISTS item_rarities (
		   ...
		   )`, `run`, [], `Verifying table item_rarities`)
		...
		await this._query(`CREATE TABLE IF NOT EXISTS commands_log (
		   ...
		   )`, `run`, [], `Verifying table commands_log`)
		await this._query(`CREATE TABLE IF NOT EXISTS resource_log (
		   ...
		   )`, `run`, [], `Verifying table resource_log`)
		return true
	}
	...
	async migrate() {
		...
		await this._query(`
			...
			VALUES('Covers', 'cov')`, `run`, [], `Register Covers type in item_types table`)

		await this._query(`
			INSERT OR IGNORE INTO item_types(name, alias)
			VALUES('Badges', 'bgs')`, `run`, [], `Register Badges type in item_types table`)

		await this._query(`
			INSERT OR IGNORE INTO item_types(name, alias)
			VALUES('Stickers', 'stc')`, `run`, [], `Register Stickers type in item_types table`)

		await this._query(`
			INSERT OR IGNORE INTO item_types(name, alias)
			VALUES('Themes', 'thm')`, `run`, [], `Register Themes type in item_types table`)

		await this._query(`
			INSERT OR IGNORE INTO item_types(name, alias, max_stacks)
			VALUES('Currencies', 'crcy', 9999999999)`, `run`, [], `Register Currencies type in item_types table`)

		await this._query(`
			INSERT OR IGNORE INTO item_types(name, alias)
			VALUES('Tickets', 'tkt')`, `run`, [], `Register Tickets type in item_types table`)

		await this._query(`
			INSERT OR IGNORE INTO item_types(name, alias)
			VALUES('Materials', 'mtrl')`, `run`, [], `Register Materials type in item_types table`)

		await this._query(`
			INSERT OR IGNORE INTO item_types(name, alias)
			VALUES('Consumables', 'cnsm')`, `run`, [], `Register Consumables type in item_types table`)

		await this._query(`
			INSERT OR IGNORE INTO item_types(name, alias)
			VALUES('Cards', 'crd')`, `run`, [], `Register Cards type in item_types table`)

		await this._query(`
			INSERT OR IGNORE INTO item_types(name, alias)
			VALUES('Gifts', 'gft')`, `run`, [], `Register Gifts type in item_types table`)

		await this._query(`
			INSERT OR IGNORE INTO item_types(name, alias)
			VALUES('Gachas', 'gch')`, `run`, [], `Register Gachas type in item_types table`)

		await this._query(`
			INSERT OR IGNORE INTO item_types(name, alias)
			VALUES('Packages', 'pkg')`, `run`, [], `Register Packages type in item_types table`)
		...
		await this._query(`
			...
			VALUES('Common', 1, '#888A91')`, `run`, [], `Register [1]Common rarity in item_rarities table`)

		await this._query(`
			INSERT OR IGNORE INTO item_rarities(name, level, color)
			VALUES('Magic', 2, '#68B73E')`, `run`, [], `Register [2]Magic rarity in item_rarities table`)

		await this._query(`
			INSERT OR IGNORE INTO item_rarities(name, level, color)
			VALUES('Rare', 3, '#99BCBF')`, `run`, [], `Register [3]Rare rarity in item_rarities table`)

		await this._query(`
			INSERT OR IGNORE INTO item_rarities(name, level, color)
			VALUES('Unique', 4, '#C21CFF')`, `run`, [], `Register [4]Unique rarity in item_rarities table`)

		await this._query(`
			INSERT OR IGNORE INTO item_rarities(name, level, color)
			VALUES('Legendary', 5, '#AD3632')`, `run`, [], `Register [5]Legendary rarity in item_rarities table`)
		...
		await this._query(`
			...
			WHERE type IN ('Roles', 'Seasonal Roles')`, `run`, [], `Deleting item with 'Roles' and 'Seasonal Roles' type in itemlist table`)

		await this._query(`
			UPDATE itemlist 
			SET type = 'Badges'
			WHERE type = 'Package Items'`, `run`, [], `Updating 'Package Items' type into 'Badges' in itemlist table`)

		await this._query(`
			UPDATE itemlist 
			SET type = 'Currencies'
			WHERE type = 'Currency'`, `run`, [], `Updating 'Currency' type into 'Currencies' in itemlist table`)

		await this._query(`
			UPDATE itemlist 
			SET type = 'Materials'
			WHERE type in ('Unique', 'Shard')`, `run`, [], `Updating 'Unique' and 'Shard' type into 'Materials' in itemlist table`)

		await this._query(`
			UPDATE itemlist 
			SET type = 'Gifts'
			WHERE type = 'Foods'`, `run`, [], `Updating 'Foods' type into 'Gifts' in itemlist table`)

		await this._query(`
			UPDATE itemlist 
			SET type = 'Consumables'
			WHERE type = 'Capsule'`, `run`, [], `Updating 'Capsule' type into 'Consumables' in itemlist table`)
		...
		await this._query(`
			...
			SELECT 
				...
				desc
			FROM itemlist`, `run`, [], `Migrating itemlist into items`)
		...
		await this._query(`
			...
			)`, `run`, [], `Registering defaultcover1 into items table`)
		...
		await this._query(`
			...
			WHERE alias = 'light'`, `run`, [], `Updating [Light Profile Theme] in items table`)
		...
		await this._query(`
			...
			SET 
				...
				type_id = (SELECT type_id FROM item_types WHERE name = 'Themes')
			WHERE alias = 'dark'`, `run`, [], `Updating [Dark Profile Theme] in items table`)
		...
		await this._query(`
			...
			WHERE name in (
				...
				'THEMES',
				'STICKERS'
			)`, `run`, [], `Updating decoration(except badges) item's max_use to 1 in item_types table`)

		await this._query(`
			UPDATE items 
			SET alias = 'lalolu_cov' 
			WHERE name = 'Lalolu'`, `run`, [], `Fixing Lalolu's Cover alias`)
		...
		await this._query(`
			...
			SELECT 
				...
				description,
				get_notification
			FROM userdata`, `run`, [], `Migrating userdata into users`)
		...
		await this._query(`
			...
			WHERE 
				...
				)`, `run`, [], `Registering all items into shop table, except for one with 'Currencies' type`)
		...
		await this._query(`
			...
			SELECT
				typeId,
				type
			FROM relationshiptype`, `run`, [], `Migrating relationshiptype into relationships`)
		await this._query(`
			INSERT INTO user_relationships (
				user_id_A,
				user_id_B,
		...
		await this._query(`
			...
			SELECT 
				...
				assigned_by,
				reason
			FROM strike_list`, `run`, [], `Migrating strike_list into strikes`)
		await this._query(`
			INSERT INTO commands_log (
				registered_at,
				user_id,
			...
			SELECT 
				...
				command_alias,
				resolved_in
			FROM commands_usage`, `run`, [], `Migrating commands_usage into commands_log`)
		await this._query(`
			...
			SELECT 
				...
				cpu,
				memory
			FROM resource_usage`, `run`, [], `Migrating resource_usage into resource_log`)
	...
	/*
	 * 
	 * Each table schema needs to be added and should have proper checks meaning "if not exists blah"
	 */
