...
class Response {
    ...
    constructor(message = {}, channelAsInstance = false) {
        ...
        this.localeSource = this.getLocale() || `en`
    ...
    async getLocale() {
        const langConstant = `LANG_${this.message.author.id || this.message.user.id}`
            //  If none of the source has user's saved locale, will fallback to locales.en
        const localeSource = await this.message.client.db.redis.exists(langConstant) ? await this.message.client.db.redis.get(langConstant) : await this.message.client.db.getUserLocale(this.message.author.id || this.message.user.id)
    ...
    async send(content = ``, plugins = {}) {
                ...
                .then(async msg => {
                    ...
                    if (cardPreviews) {
                        ...
                        preview.on(`collect`, async r => {
                            ...
                            field.send({ content: ``, files: new MessageAttachment(img) })
                            loading.delete()
                        })
                    }
                    //	Left navigation
        ...
        if (typeof content != `string`) content = this.message.client.locales[this.localeSource].LOCALIZATION_ERROR
            //  Find all the available {{}} socket in the string.
        let sockets = content.match(/\{{(.*?)\}}/g)
        if (sockets === null) sockets = []
        for (let i = 0; i < sockets.length; i++) {
        ...
        if (simplified) return field.send({
                content: content,
                files: [
        ...
        let sent = field.send({ content: topNotch, embeds: [embed] })
