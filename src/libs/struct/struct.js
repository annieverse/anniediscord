...
cstruct.packSync = function (name, object, options) {
  ...
  return new Buffer.from(next(object, schema));
};
...
packer.uint24b = function (val) {
  var binary = [];
  binary[0] = (val >> 16) & 0xFF;
  binary[1] = (val >> 8) & 0xFF;
  binary[2] = (val) & 0xFF;
  return binary;
};
...
packer.uint32b = function (val) {
  var binary = [];
  binary[0] = (val >> 24) & 0xFF;
  binary[1] = (val >> 16) & 0xFF;
  binary[2] = (val >> 8) & 0xFF;
  binary[3] = (val) & 0xFF;
  return binary;
};
...
packer.uint48b = function (val) {
  var binary = [];
  binary[0] = (val / 0x10000000000) & 0xFF;
  binary[1] = (val / 0x100000000) & 0xFF;
  binary[2] = (val >> 24) & 0xFF;
  binary[3] = (val >> 16) & 0xFF;
  binary[4] = (val >> 8) & 0xFF;
  binary[5] = (val) & 0xFF;
  return binary;
};
...
unpacker.boolean = function (pointer, buffer) {
  return buffer[pointer.offset++] == 0 ? false : true;
};
...
unpacker.uint8b = function (pointer, buffer) {
  return buffer[pointer.offset++];
};
...
unpacker.uint16b = function (pointer, buffer) {
  var binary = buffer[pointer.offset++] << 8;
  binary |= buffer[pointer.offset++];
  binary >>>= 0;
  return binary;
};
...
unpacker.uint24b = function (pointer, buffer) {
  var binary = buffer[pointer.offset++] << 16;
  binary |= buffer[pointer.offset++] << 8;
  binary |= buffer[pointer.offset++];
  binary >>>= 0;
  return binary;
...
unpacker.uint32b = function (pointer, buffer) {
  var binary = buffer[pointer.offset++] << 24;
  binary |= buffer[pointer.offset++] << 16;
  binary |= buffer[pointer.offset++] << 8;
  binary |= buffer[pointer.offset++];
  binary >>>= 0;
...
unpacker.uint40b = function (pointer, buffer) {
  var binary = buffer[pointer.offset++] * 0x100000000;
  binary >>>= 0;
  binary |= buffer[pointer.offset++] << 24;
  binary |= buffer[pointer.offset++] << 16;
  binary |= buffer[pointer.offset++] << 8;
...
unpacker.uint48b = function (pointer, buffer) {
  var binary = buffer[pointer.offset++] * 0x10000000000;
  binary += buffer[pointer.offset++] * 0x100000000;
  binary >>>= 0;
  binary |= buffer[pointer.offset++] < 24;
  binary |= buffer[pointer.offset++] << 16;
...
cstruct.packers = packer;
cstruct.unpackers = unpacker;