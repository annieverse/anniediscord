...
class UI {
    ...
    async build() {
            const avatar = await loadImage(this.member.user.displayAvatarURL({ extension: `png`, forceStatic: true }))
            let canvas_x = 800
            let canvas_y = 250
            let start_x = 30
            let start_y = 30
            ...
            const background = await loadAsset(welcomerBackgroundId, {assetsPath:`./src/assets/customWelcomer`})
            if (noWelcomerImage) return
            let canv = new Canvas(800, 250)
            const imageCanv = Buffer.from((new Canvas(avatar.width, avatar.height).printImage(avatar, 0, 0).png()).buffer)
            const {
                width: bgWidth,
                height: bgHeight
            ...
            .printImage(userPfpEnabled ? avatar : await loadImage(background), 0, 0, dynamic.width, dynamic.height)
