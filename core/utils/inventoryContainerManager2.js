...
async function filtering_items(container) {
	...
	const eliminate_nulls = async () => {
		for (let i in bag) {
			if (bag[i].quantity === null || bag[i].quantity === undefined || bag[i].quantity < 1) {
				delete bag[i]
			}
		}
	}
	...
	const name_labeling = async () => {
		for (let i in bag) {
			sql.get(`SELECT name FROM itemlist WHERE itemId = ${bag[i].item_id}`)
				.then(async data => parsedbag[data.name] = bag[i].quantity)
		}
	}

	// Store rarity of the item.
	const get_alias = async () => {
		for (let i in bag) {
			sql.get(`SELECT alias FROM itemlist WHERE itemId = ${bag[i].item_id}`)
				.then(async data => aliasbag[i] = data.alias)
		}
	}
	...
	const get_rarities = async () => {
		for (let i in bag) {
			sql.get(`SELECT rarity FROM itemlist WHERE itemId = ${bag[i].item_id}`)
	...
	return {
		filter_res: msg,
		filter_oringal_res: bag,
		filter_alias_res: aliasbag,
