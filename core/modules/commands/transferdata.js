	...
	async execute() {
		...
		try {
			...
			for (let index = 0; index < userdataData.length; index++) {
				...
				item = userdataData[index].slot2
				if (item != null) {
					let coverId = await db._query(`SELECT itemId FROM itemlist WHERE alias = ?`, `get`, [item])
					await db._transforInventory({ itemId: coverId.itemId, value: 1, userId: userdataData[index].userId })
				}

				item = userdataData[index].slot3
				if (item != null) {
					let coverId = await db._query(`SELECT itemId FROM itemlist WHERE alias = ?`, `get`, [item])
					await db._transforInventory({ itemId: coverId.itemId, value: 1, userId: userdataData[index].userId })
				}

				item = userdataData[index].slot4
				if (item != null) {
					let coverId = await db._query(`SELECT itemId FROM itemlist WHERE alias = ?`, `get`, [item])
					await db._transforInventory({ itemId: coverId.itemId, value: 1, userId: userdataData[index].userId })
				}

				item = userdataData[index].slot5
				if (item != null) {
					let coverId = await db._query(`SELECT itemId FROM itemlist WHERE alias = ?`, `get`, [item])
					await db._transforInventory({ itemId: coverId.itemId, value: 1, userId: userdataData[index].userId })
				}

				item = userdataData[index].slot6
				if (item != null) {
					let coverId = await db._query(`SELECT itemId FROM itemlist WHERE alias = ?`, `get`, [item])
					await db._transforInventory({ itemId: coverId.itemId, value: 1, userId: userdataData[index].userId })
				}
				item = userdataData[index].slotanime
				if (item != null) {
					let coverId = await db._query(`SELECT itemId FROM itemlist WHERE alias = ?`, `get`, [item])
					await db._transforInventory({ itemId: coverId.itemId, value: 1, userId: userdataData[index].userId })
				}
