  ...
  const switchColor = {
    ...
    canv = canv.setColor(user.clr)
           ...
           //BADGES
               ...
               // first row
               //.addImage(await getBadge('mdb'), startPos_x+150, startPos_y+222, 40, 40, 20)
               //.addImage(await getBadge('sai'), startPos_x+90, startPos_y+222, 40, 40, 20)           
               //.addImage(await getBadge('ps'), startPos_x+30, startPos_y+222, 40, 40, 20)

               //second row
               //.addImage(await getBadge('csp'), startPos_x+407, startPos_y+222, 40, 40, 20)
               //.addImage(await getBadge('csp'), startPos_x+467, startPos_y+222, 40, 40, 20)
               //.addImage(await getBadge('ps'), startPos_x+527, startPos_y+222, 40, 40, 20)
           ...
           .setTextAlign("left")  // current xp, max xp & percentage
            if (user.crv === 150){
             canv.addText(`${configFormat.threeDigitsComa(user.cur)} / ${configFormat.threeDigitsComa(user.max)} (${configFormat.getPercentage(PanCurrent, user.max)}%)`, startPos_x+60, 540) // xp informations
            } else {
              console.log(user.cur);
              console.log(user.crv);
              console.log(user.max);
              console.log(PanCurrent);
              canv.addText(`${configFormat.threeDigitsComa(user.cur)} / ${configFormat.threeDigitsComa(user.max)} (${configFormat.getPercentage(PanCurrent, user.crv)}%)`, startPos_x+60, 540) // xp informations
            }
           canv.setTextFont("14pt Roboto") // required exp to next lvl
           .setTextAlign("left")
...
try {  
    const getUser = new userFinding.userFinding(message, message.content.substring(9))
