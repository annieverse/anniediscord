...
return ["sandbox"].includes(message.channel.name) ? init_gift()
   ...
   class Transaction {
        ...
        get add_reps() {
            return sql.run(`UPDATE userdata
                            SET reputations = reputations + ${this.metadata.counted_reps}
        ...
        log({code: `PROMPT_INVENTORY`}, textified(metadata), metadata.target.name)
            .then(async prompt_msg => {
                collector.on(`collect`, async (msg) => {
                    ...
                    metadata.amount_to_send = parseInt(user_input);
