...
const databaseManager = require(`../../this.utils/databaseManager`)
const formatManager = require(`../../this.utils/formatManager`)

class dbkits {
	constructor(Stacks) {
		this.utils = Stacks.this.utils
	...
	async execute() {
		...
		/**
         * dbkits.
         * @this.utils direct edit/view database.
		...
		async function modifyDB() {
			...
			if (args[0] === `query`) {
                 ...
                 */
				if (args[1]) {
					const user = await this.utils.userFinding(message, args[1])
					let data = {
						id: user.id,
						tbl: args[2],
						get parsed() {
				...
				if (args[1]) {
					...
					let tokenize = message.content
						.substring(this.command.length + 15)
						.split(`, `)
				...
				if (args[1]) {
					try {
                         ...
                         */
						let tokenize = message.content
							.substring(this.command.length + 10)
							.split(`, `)
						console.log(tokenize)
						const user = await this.utils.userFinding(message, tokenize[0])
						let data = {
